...
public class Minimap
{
    ...
    public static int loadBlockColourFromTexture(final IBlockState state, final Block b, final BlockPos pos, final boolean convert) {
        ...
        if (c == null) {
            ...
            try {
                ...
                c = b.getMapColor(state, mc.world, pos).colorValue;
                if (name != null) {
                    Minimap.textureColours.put(name, c);
                }
        ...
        final int grassColor = b.getMapColor(state, Minecraft.getMinecraft().world, pos).colorValue;
    ...
    public boolean applyTransparentBlock(final Chunk bchunk, final Block b, final IBlockState state, final BlockPos globalPos, final BlockPos pos) {
        ...
        int intensity = 1;
        boolean skip = false;
        if (b instanceof BlockLiquid && b.getLightOpacity(state) != 255 && b.getLightOpacity(state) != 0) {
        ...
        if (skip) {
            ...
            this.sun -= b.getLightOpacity(state);
            if (this.sun < 0) {
                this.sun = 0;
            }
        }
    ...
    public Block findBlock(final Chunk bchunk, final int insideX, final int insideZ, final int highY, final int lowY) {
        ...
        for (int i = highY; i >= lowY; --i) {
            IBlockState state1 = bchunk.getBlockState(insideX, i, insideZ);
            final IBlockState got = state1;
            if (!(got instanceof BlockAir) && (underair || this.loadingCaving == -1)) {
                if (state1.getRenderType() != EnumBlockRenderType.INVISIBLE) {
                    if (got != Blocks.TORCH) {
                        if (got != Blocks.TALLGRASS) {
                            this.blockY = i;
                            int color = 0;
                            final BlockPos pos = new BlockPos(insideX, this.blockY, insideZ);
                            final BlockPos globalPos = this.getGlobalBlockPos(bchunk.x, bchunk.z, insideX, this.blockY, insideZ);
                            IBlockState state = bchunk.getBlockState(pos);
                        	state = state1.getActualState(mc.world, globalPos);
                            if (!this.applyTransparentBlock(bchunk, got.getBlock(), state, globalPos, pos)) {
                                color = loadBlockColourFromTexture(state, got.getBlock(), globalPos, true);
                                if (color != 0) {
    ...
    public boolean isGlowing(final Block b) {
        return b.getLightValue(b.getDefaultState()) >= 0.5;
    }
    ...
    public void loadBlockColor(final int par1, final int par2, final Chunk bchunk, final int chunkX, final int chunkZ) {
        ...
        final int playerY = (int)Minimap.mc.player.posY;
        ...
        if (chunk == null) {
            chunk = new MinimapChunk(bchunk.x, bchunk.z);
    ...
    public void updateMapFrame(final int bufferSize, final float partial) {
        ...
        if (usingFBO()) {
            ...
            Minimap.mapUpdateX = this.getMapCoord(actualSize, getEntityX(Minimap.mc.player, partial));
            Minimap.mapUpdateZ = this.getMapCoord(actualSize, getEntityZ(Minimap.mc.player, partial));
    ...
    private int getCaving() {
        final int x = myFloor(Minimap.mc.player.posX);
        final int y = Math.max((int)Minimap.mc.player.posY + 1, 0);
        final int z = myFloor(Minimap.mc.player.posZ);
        ...
        if (y < top) {
            for (int i = y; i < top; ++i) {
                final IBlockState got = bchunk.getBlockState(insideX, i, insideZ);
    ...
    public void renderFrameToFBO(final int bufferSize, final int viewW, final float sizeFix, final float partial, final boolean retryIfError) {
        ...
        double xInsidePixel = getEntityX(Minimap.mc.player, partial) - xFloored;
        if (xInsidePixel < 0.0) {
            ++xInsidePixel;
        }
        double zInsidePixel = getEntityZ(Minimap.mc.player, partial) - zFloored;
        if (zInsidePixel < 0.0) {
            ++zInsidePixel;
        }
    ...
    public class MapLoader implements Runnable
    {
        ...
        public void run() {
            ...
            while (true) {
                ...
                boolean sleep = true;
                try {
                    if (Minimap.mc.player == null || Minimap.mc.world == null) {
                        Thread.sleep(100L);
                    }
                    else {
                        if (updateChunkX == 0 && updateChunkZ == 0) {
                            ...
                            Minimap.this.loadingMapX = Minimap.this.getMapCoord(Minimap.loadingSide, Minimap.mc.player.posX);
                            Minimap.this.loadingMapZ = Minimap.this.getMapCoord(Minimap.loadingSide, Minimap.mc.player.posZ);
                        ...
                        if (updateChunkZ == 0) {
                            ...
                            final EntityPlayer p = Minimap.mc.player;
                            ...
                            for (int i = 0; i < mc.world.loadedEntityList.size(); ++i) {
                                try {
                                    final Entity e = mc.world.loadedEntityList.get(i);
                                    int type = 0;
